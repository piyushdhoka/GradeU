...
export const ProctoringComponent: React.FC<ProctoringComponentProps> = ({
  ...
  return (
    <div className="fixed right-4 bottom-4 z-50">
      <div
        className={`relative overflow-hidden rounded-lg border-2 shadow-2xl transition-colors duration-300 ${status === 'violation' ? 'border-red-500 shadow-red-500/50' : 'border-[#00FF88] shadow-[#00FF88]/20'}`}
      >
        {/* Status Overlay */}
        <div className="absolute top-0 right-0 left-0 z-10 flex items-center justify-between bg-black/70 p-2 text-xs font-bold backdrop-blur-sm">
          ...
          <div
            className={`flex items-center space-x-1 ${status === 'violation' ? 'text-red-500' : 'text-[#00FF88]'}`}
          >
            {status === 'violation' ? (
        ...
        <div className="relative h-36 w-48 bg-black">
          <video
            ...
            muted
            playsInline
            className="h-full w-full scale-x-[-1] transform object-cover" // Mirror effect
          />
          {!modelLoaded && (
