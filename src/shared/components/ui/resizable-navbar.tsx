    ...
    >
      {items.map((item, idx) => (
        <button
          ...
          className="hover:text-brand-400 pointer-events-auto relative cursor-pointer px-4 py-2 text-zinc-400 transition-colors"
          key={`link-${idx}`}
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="bg-brand-400/10 absolute inset-0 h-full w-full rounded-lg"
            />
          )}
          <span className="relative z-20 text-xs font-bold tracking-[0.15em] uppercase">
            {item.name}
          </span>
        </button>
      ))}
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0, y: -10 }}
          className={cn(
            'border-brand-400/20 absolute inset-x-0 top-full z-50 mt-2 flex w-full flex-col items-start justify-start gap-4 rounded-2xl border bg-black/95 px-6 py-8 backdrop-blur-xl',
            className
          )}
        >
          {children}
...
export const MobileNavToggle = ({
  ...
  return isOpen ? (
    <IconX className="text-brand-400 h-6 w-6 cursor-pointer" onClick={onClickAction} />
  ) : (
    <IconMenu2 className="text-brand-400 h-6 w-6 cursor-pointer" onClick={onClickAction} />
  );
};
...
export const NavbarLogo = ({ onClickAction }: { onClickAction?: () => void }) => {
  return (
    <button
      ...
      className="group relative z-20 flex cursor-pointer items-center gap-3"
    >
      ...
      <span className="hidden font-sans text-2xl font-black tracking-tighter text-white uppercase sm:block">
...
export const NavbarButton = ({
  ...
  const variantStyles = {
    primary: 'bg-brand-400 text-black hover:bg-brand-500 hover:scale-105',
    secondary: 'bg-transparent text-zinc-400 hover:text-brand-400',
    outline:
      'bg-transparent border border-brand-400/30 text-brand-400 hover:bg-brand-400/10 hover:border-brand-400',
  };
