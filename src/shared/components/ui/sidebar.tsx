...
>(
  (
    ...
    // We use hook() instead of create() because we want to be able to read it outside of React.
  ...
  (
    ...
    return (
      ...
      >
        ...
        <div
          className={cn(
            'fixed inset-y-0 z-10 hidden h-svh w-64 content-stretch transition-[left,right,width] duration-200 ease-linear md:flex',
            side === 'left'
            ...
            variant === 'floating' || variant === 'inset'
              ? 'group-data-[collapsible=icon]:w-[calc(3rem_+_theme(spacing.4)_+2px)]'
              : 'border-sidebar-border border-r group-data-[collapsible=icon]:w-[3rem]',
            className
          )}
          {...props}
        >
...
>(({ className, asChild = false, showOnHover = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        'text-sidebar-foreground ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground absolute top-1.5 right-1 flex aspect-square w-5 items-center justify-center rounded-md p-0 transition-transform outline-none focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
        // Increases the hit area of the button on mobile.
        'after:absolute after:-inset-2 md:after:hidden',
